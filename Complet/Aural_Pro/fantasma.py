...
def ruta_dades_usuari(nom_fitxer):
    """Ruta writable per a fitxers de dades d'usuari.
    - En mode exe: al costat de l'executable (o a %APPDATA%)
    - En mode script: al directori del script (comportament actual)
    """
    if getattr(sys, 'frozen', False):
        # Estem dins l'exe compilat → carpeta on hi ha l'exe
        base = os.path.dirname(sys.executable)
    else:
        # Estem executant el .py normal
...
DESCANS_MISSATGES = [
    ...
    "Bebe un got d'aigua, t'ho has guanyat 💧",
    "Fes 10 flexions o esquats ràpids 💪",
    "Menja alguna cosa lleugera 🍎",
    "Mira lluny un moment, descansa la vista 👀",
    "Respira profundament 3 vegades 🌬️",
    "Obre la finestra i agafa aire fresc 🌤️",
