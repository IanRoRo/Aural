...
def ruta_recurs(nom_fitxer):
    """Retorna la ruta absoluta d'un recurs, compatible amb .exe Onefile."""
...
FITXER_MEMORIA = ruta_dades_usuari("memoria_aural_pro.json")
...
def assegurar_ollama():
    try:
        ...
        print("⚠️ ERROR: Ollama no està actiu. DeepSeek-R1 requereix Ollama.")

def parsejar_resposta_deepseek(contingut):
    """DeepSeek-R1 retorna el raonament entre tags <think>...</think>"""
    match = re.search(r'</think>\s*(.*)', contingut, re.DOTALL)
    if match:
...
def determinar_categoria_ia(objectiu_usuari):
    ...
    try:
        ...
        contingut_raw = response["message"]["content"].strip()
        contingut = parsejar_resposta_deepseek(contingut_raw)
        cat = contingut.lower()
...
def buscar_perfil_similar(objectiu_usuari):
    ...
    try:
        ...
        Does any existing profile match this goal closely?
...
def guardar_feedback_usuari(objectiu_usuari, categoria_correcta):
    ...
    print(f"💾 [FEEDBACK PRO] '{objectiu_min}' -> categoria '{categoria_correcta}'.")
...
def avaluar_amb_ia(objectiu_usuari, titol_finestra):
    ...
    try:
        ...
        if "|" in contingut:
            ...
            if estat == "DISTRET":
                ...
                if (
                    ...
                    guardar_memoria(memoria)
                    print(f"\n🛑 [LLISTA NEGRA PRO] Afegit '{paraula_brossa}' a '{categoria}'.")
        ...
        return "PRODUCTIU", f"Error de connexió amb DeepSeek-R1: {e}"